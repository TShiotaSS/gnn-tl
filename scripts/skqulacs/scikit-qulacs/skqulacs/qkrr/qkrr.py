...
class QKRR:
    ...
    def __init__(self, circuit: LearningCircuit, n_iteration=50, alpha=1.0) -> None:
        """
    ...
    def fit_with_no_optimize_kernel(self, x: NDArray[np.float_], y: NDArray[np.int_]) -> None:
        """
        train the machine.
        :param x: training inputs
        :param y: training teacher values
    ...
    def predict_with_no_optimize_kernel(self, xs: NDArray[np.float_]) -> NDArray[np.float_]:
        """
        predict y values for each of xs
        :param xs: inputs to make predictions
        :return: List[int], predicted values of y
